...
export function useNotificationCenter<Data = any>(
  params: UseNotificationCenterParams<Data> = {}
): UseNotificationCenter<Data> {
  const sortFn = useRef<SortFn<Data>>(params.sort || defaultSort)
  ...
  useEffect(() => {
    return toast.onChange((toast) => {
      if ((toast.data as any).isHideInNC) return
...
function decorate<Data>(item: NotificationCenterItem<Data> | Partial<NotificationCenterItem<Data>>) {
...
// newest to oldest
function defaultSort<T = {}>(l: NotificationCenterItem<T>, r: NotificationCenterItem<T>) {
