import { FC, forwardRef, InputHTMLAttributes, ReactNode, useContext, useEffect, useRef } from 'react'
...
const DatePicker = ({ children, options, onChange, classNames, show, placeholder, setShow }: IDatePickerProps) => (
...
const DatePickerMain: FC<DatePickerMainProps> = ({ children, placeholder }) => {
...
const Input = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement>>((_props, ref) => {
  ...
  return (
    <input
      ...
      id="date"
      className={twMerge(
        'pl-9 pr-2.5 py-3 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500',
        options?.theme?.input
      )}
